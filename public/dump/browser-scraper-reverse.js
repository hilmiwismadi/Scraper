/**
 * Instagram Browser-Based Scraper - REVERSE MODE (Last Posts)
 *
 * This script runs in browser console to:
 * 1. Scroll to target depth (lazy loading)
 * 2. Scrape last N posts going UPWARD (from newest loaded post to older)
...
(function() {
  ...
  let postsToScrapeFromEnd = 10;  // NEW: Number of posts to scrape from the end
  let targetUsername = '';
  const collectedUrls = [];
  const scrapedPosts = [];
  const diagnosticLog = [];
  ...
  async function runScrapePhase(postUrls) {
    log('=== Phase 2: Scraping Last Posts (Reverse Order) ===');
    log(`Total posts loaded: ${postUrls.length}`);
  ...
  function saveResults() {
    const results = {
      metadata: {
        ...
        mode: 'reverse'  // NEW: Indicates reverse mode scraping
      },
      posts: scrapedPosts,
  ...
  async function showResults(results) {
    ...
    log(`Mode: Reverse (last ${results.metadata.postsToScrapeFromEnd} posts)`);
    log('');
    log('📊 Results Summary:');
  ...
  async function main() {
    log('=== Instagram Browser-Based Scraper - REVERSE MODE ===');
    log('🔄 Scrapes LAST posts going UPWARD');
    log('');
    ...
    try {
      ...
      // Phase 2: Ask user how many posts to scrape from the end
      log('');
      ...
      const countInput = prompt(
        ...
        'Examples:\n' +
        `  • 10 = Scrape last 10 posts (index ${Math.max(0, postUrls.length - 10)} to ${postUrls.length - 1})\n` +
        `  • 20 = Scrape last 20 posts\n` +
        `  • 50 = Scrape last 50 posts\n` +
        `  • ${postUrls.length} = Scrape all ${postUrls.length} posts\n\n` +
      ...
      // Validate input
      if (postsToScrapeFromEnd > postUrls.length) {
        log(`⚠ Requested ${postsToScrapeFromEnd} posts but only ${postUrls.length} available`);
        postsToScrapeFromEnd = postUrls.length;
      }
      ...
      // Phase 3: Scrape posts in reverse order
