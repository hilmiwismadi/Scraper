...
## 📤 After Parsing: Upload & View Results

**Step-by-Step for User:**

1. **Copy Claude's CSV output** (the full CSV code block)
2. **Replace the original CSV file** with the new content:
   - File location: `parsed/parsed#{increment}-{sessionId}-{timestamp}.csv`
   - Example: `parsed/parsed#1-local-1768176515866.csv`
3. **Refresh the web interface** at:
   ```
   http://localhost:3003/parse-manager.html?file=scraped-{sessionId}-{timestamp}.json
   ```
4. **View parsed results** - The table will now display:
   - Event Title, Organizer, Date, Location, Fee, Contacts
   - Status badges (parsed/partial/non-event)
5. **Edit if needed** - You can edit fields directly in the web interface
6. **Click "Send to VPS"** when satisfied
